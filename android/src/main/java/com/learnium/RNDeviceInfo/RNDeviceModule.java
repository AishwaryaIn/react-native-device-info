 ...
 private static final int LOCK_REQUEST_CODE = 221;
  ...
  @Override
    public void onActivityResult(Activity activity,int requestCode, int resultCode, Intent data) {
      Log.d("Test", "In side Activity result");
      if (mPickerPromise != null) {  
        if (resultCode == Activity.RESULT_CANCELED) {
          mPickerPromise.reject("User cancelled");
        } else if (resultCode == Activity.RESULT_OK) {
          mPickerPromise.resolve("Successfully.");
        }
      }
      mPickerPromise = null;
    }
  ...
  public RNDeviceModule(ReactApplicationContext reactContext) {
    super(reactContext);
    reactContext.addActivityEventListener(mActivityEventListener);//Device auth
...
// **************************** Device auth implements ActivityEventListener *******************************//
